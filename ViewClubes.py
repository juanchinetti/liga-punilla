...
def obtener_clubes(filtro_genero=None, filtro_nombre=None):
    try:
        ...
        # Filtro por género y nombre del club
        query = """
        ...
        if filtro_genero or filtro_nombre:
            query += " WHERE"
            if filtro_genero:
                query += " g.descripcion = %s"
...
def actualizar_treeview():
    ...
    if filtro_genero == "Todos":
        filtro_genero = None
    filtro_nombre = entry_buscar.get()  # Obtener el texto del cuadro de búsqueda
    clubes_bd = obtener_clubes(filtro_genero, filtro_nombre)  # Obtener clubes desde la base de datos con filtro
...
arbol.heading("tipo", text="Categoria")
