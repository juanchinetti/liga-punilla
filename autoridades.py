...
conexion = mysql.connector.connect(
    ...
    password="tomilola23.",  # Tu contraseña de MySQL
...
# Función para mostrar autoridades en la lista
def mostrar_autoridades(autoridades=None):
    lista_autoridades.delete(0, tk.END)
    if autoridades is None:
        autoridades = cargar_autoridades()
    
    for autoridad in autoridades:
        lista_autoridades.insert(tk.END, f"{autoridad['id']}. {autoridad['nombre']} {autoridad['apellido']} - {autoridad['puesto']}")
...
def modificar_autoridad():
    try:
        seleccion = lista_autoridades.curselection()[0]
        autoridad_seleccionada = lista_autoridades.get(seleccion)
        id_autoridad = int(autoridad_seleccionada.split('.')[0])
        ...
        messagebox.showerror("Error", "Debe seleccionar una autoridad para modificar.")
    except mysql.connector.Error as err:
...
def baja_autoridad():
    try:
        ...
        messagebox.showerror("Error", "Debe seleccionar una autoridad para eliminar.")
    except mysql.connector.Error as err:
...
def buscar_por_puesto(*args):
    ...
    cursor.close()
    mostrar_autoridades(autoridades_encontradas)
...
btn_mostrar_todas = tk.Button(frame_controles, text="Mostrar Todas", command=mostrar_autoridades)
...
# Lista para mostrar autoridades
lista_autoridades = tk.Listbox(ventana, height=20, width=100)
lista_autoridades.pack(pady=20)

# Botón para volver al menú
btn_volver_menu = tk.Button(ventana, text="Volver al Menú", command=Volver_menu)
btn_volver_menu.pack(pady=10)

# Cargar las autoridades al iniciar
mostrar_autoridades()
